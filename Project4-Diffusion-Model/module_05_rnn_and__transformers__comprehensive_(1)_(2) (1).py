...
Original file is located at
    https://colab.research.google.com/drive/1-JIeam1ej5Hg-vkpKtv2VyhGhsM1SWHz
...
import torch.nn as nn # Added to resolve NameError
...
class VanillaRNNClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # GRU and RNN return (output, hidden) - no cell state
        output, hidden = self.rnn(embedded)

        # Concatenate final forward and backward hidden states
        # hidden: (num_layers * num_directions, batch_size, hidden_size)
        # For bidirectional, hidden[-2] is last forward, hidden[-1] is last backward
        hidden = torch.cat((hidden[-2], hidden[-1]), dim=1)
...
# Train the vanilla RNN model for 3 epochs as per exercise instructions
print("\n" + "=" * 60)
print("Training Vanilla RNN")
print("=" * 60)

rnn_history = train_model(vanilla_rnn_model, train_loader, test_loader, epochs=3, name="Vanilla RNN")

print("\nPer-class accuracy (Vanilla RNN):")
rnn_class_acc = eval_per_class(vanilla_rnn_model, test_loader, CLASS_NAMES)

# After training, compare results:
print("\n=== RNN Architecture Comparison ===")
print(f"Vanilla RNN Test Accuracy: {rnn_history['test_acc'][-1]:.4f}%")
print(f"LSTM Test Accuracy:        {lstm_history['test_acc'][-1]:.4f}%")  # From earlier
print(f"GRU Test Accuracy:         {gru_history['test_acc'][-1]:.4f}%")  # From earlier

rnn_model = VanillaRNNClassifier(len(word2idx), EMBED_DIM, HIDDEN_DIM, NUM_CLASSES).to(device)

print("Training Vanilla RNN")
rnn_history = train_model(rnn_model, train_loader, test_loader, epochs=3)

rnn_acc = rnn_history['test_acc'][-1]
...
"""**2. Run `Cell A.10` (to train the LSTM model and define `lstm_history`):**"""
...
"""**3. Run `Cell A.11` (to train the GRU model and define `gru_history`):**"""
...
for hidden_dim in [64, 128]:
    for dropout in [0.1, 0.3]:
        for lr in [0.001, 0.0005]:
            ...
            results.append((hidden_dim, dropout, lr, acc))

            print("\nHidden | Dropout | LR     | Accuracy")
print("-------------------------------------")
for r in results:
    print(f"{r[0]:<6} | {r[1]:<7} | {r[2]:<6} | {r[3]:.4f}")
...
# Load the configuration first and set output_attentions to True
...
# ✂️ YOUR CODE HERE: Extract and visualize attention maps
...
def extract_attention_maps(model, image, layer_idx=-1):
    ...
    Returns:
        attention_map: 2D attention weights for visualization (averaged across heads),
                       reshaped to be square for a single image, excluding CLS token.
                       Shape: (num_patches_sqrt, num_patches_sqrt)
    """
    model.eval()
    with torch.no_grad():
        ...
        attentions = outputs.attentions # This should now be a tuple of tensors

    # Get attention from the specified layer
    if attentions is None or len(attentions) == 0:
        print("Error: No attention weights returned. Ensure model is configured to output attentions.")
        return None

    # The attention tuple contains 12 layers of attention weights.
    # Each item in the tuple has shape (batch_size, num_heads, seq_len, seq_len)
    # We take the attention from the specified layer (default is last layer)
    layer_attention = attentions[layer_idx]

    # Average attention across all heads: (batch_size, seq_len, seq_len)
    mean_attention = layer_attention.mean(dim=1)

    # We are interested in the attention of the [CLS] token (index 0) to all other patch tokens.
    # The first token is [CLS], so we take its attention to all subsequent tokens (patches).
    # Shape: (batch_size, seq_len - 1)
    cls_to_patches_attention = mean_attention[:, 0, 1:]

    # Reshape the attention to a square grid corresponding to the image patches.
    # For a 224x224 image with 16x16 patches, there are (224/16)*(224/16) = 14*14 = 196 patches.
    # So, seq_len - 1 = 196, and sqrt(196) = 14.
    num_patches_per_dim = int(cls_to_patches_attention.size(-1)**0.5)

    # Assuming batch_size = 1 for this visualization function
    attention_map = cls_to_patches_attention.view(num_patches_per_dim, num_patches_per_dim).cpu().numpy()

    return attention_map

def visualize_attention(image, attention_map, class_name, ax_img, ax_attn, pred_class_name=None, true_class_name=None):
    """
    Create a visualization overlaying attention on the original image.

    Args:
        image: Original image tensor (C, H, W) in [0, 1] range.
        attention_map: 2D attention weights (e.g., 14x14) for visualization.
        class_name: Class label for the image (for title).
        ax_img: Matplotlib Axes object for the image.
        ax_attn: Matplotlib Axes object for the attention map.
        pred_class_name: Predicted class name (optional, for title).
        true_class_name: True class name (optional, for title).
    """
    # Denormalize image for display (assuming ImageNet normalization)
    # Reversing: img * std + mean
    mean = np.array([0.485, 0.456, 0.406])
    std = np.array([0.229, 0.224, 0.225])
    img_display = image.permute(1, 2, 0).cpu().numpy() * std + mean
    img_display = np.clip(img_display, 0, 1)

    ax_img.imshow(img_display)
    ax_img.axis('off')
    title_str = f"True: {true_class_name}\nPred: {pred_class_name}" if pred_class_name and true_class_name else f"Class: {class_name}"
    ax_img.set_title(title_str, fontsize=10)

    # Resize attention map to image dimensions for overlay
    # The attention map is small (e.g., 14x14), need to upscale it.
    # Using torch.nn.functional.interpolate for resizing
    attention_map_tensor = torch.from_numpy(attention_map).unsqueeze(0).unsqueeze(0) # (1, 1, H, W)
    upscaled_attention_map = F.interpolate(attention_map_tensor, size=img_display.shape[:2], mode='bilinear', align_corners=False)
    upscaled_attention_map = upscaled_attention_map.squeeze().cpu().numpy()

    ax_attn.imshow(img_display)
    ax_attn.imshow(upscaled_attention_map, cmap='hot', alpha=0.5) # Overlay with transparency
    ax_attn.set_title('Attention Map Overlay', fontsize=10)
    ax_attn.axis('off')

# --- Code to select images and visualize attention maps ---

# Select 5 images from different classes for visualization
# Using indices to ensure diverse classes if available in the test subset
visualize_indices = []
selected_classes = set()

# Iterate through the test dataset to find diverse images
for i in range(len(cifar_test)):
    _, label = cifar_test[i]
    if label not in selected_classes:
        visualize_indices.append(i)
        selected_classes.add(label)
    if len(selected_classes) >= 5: # Get at least 5 unique classes
        break

# If less than 5 unique classes are found, just use the first 5 images
if len(visualize_indices) < 5:
    visualize_indices = list(range(min(5, len(cifar_test))))

fig, axes = plt.subplots(len(visualize_indices), 2, figsize=(8, 4 * len(visualize_indices)))

if len(visualize_indices) == 1:
    axes = [axes] # Make axes iterable if only one image

for i, idx in enumerate(visualize_indices):
    image, true_label_idx = cifar_test[idx]
    true_class_name = CIFAR_CLASSES[true_label_idx]

    # Get model prediction
    vit_model.eval()
    with torch.no_grad():
        outputs = vit_model(image.unsqueeze(0).to(device))
        logits = outputs.logits
        predicted_label_idx = logits.argmax(-1).item()
        predicted_class_name = CIFAR_CLASSES[predicted_label_idx]

    # Extract attention map
    attention_map = extract_attention_maps(vit_model, image)

    if attention_map is not None:
        ax_img = axes[i, 0] if len(visualize_indices) > 1 else axes[0] # Handle single row subplot
        ax_attn = axes[i, 1] if len(visualize_indices) > 1 else axes[1]
        visualize_attention(image, attention_map, true_class_name,
                            ax_img, ax_attn, predicted_class_name, true_class_name)

plt.tight_layout()
plt.suptitle('ViT Attention Map Visualization', fontsize=16, y=1.02)
plt.show()
...
# C.4 — Visualize ViT Attention Maps
...
last_attn = attentions[-1].cpu().mean(dim=1)  # average over heads
...
class CNNClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(torch.relu(self.conv2(x)))   # (64, 8, 8)
...
# Initialize with available data and placeholders for upcoming parts
# Please fill in the '???' values once you complete Parts B and C.
results_summary = {
    'Vanilla RNN': {
        'Task': 'Text Classification',
        'Test Accuracy': rnn_history['test_acc'][-1] * 100, # Convert to percentage
        'Training Time (s)': sum(rnn_history['epoch_times']),
        'Parameters': sum(p.numel() for p in vanilla_rnn_model.parameters()),
        'Strengths': 'Simpler, faster to implement',
        'Weaknesses': 'Vanishing gradients, poor long-term memory',
        'Best Use Cases': 'Very short sequences, quick baselines'
    },
    'LSTM': {
        'Task': 'Text Classification',
        'Test Accuracy': lstm_history['test_acc'][-1] * 100,
        'Training Time (s)': sum(lstm_history['epoch_times']),
        'Parameters': sum(p.numel() for p in lstm_model.parameters()),
        'Strengths': 'Handles long-term dependencies, good for sequences',
        'Weaknesses': 'Slower than GRU, more complex gating',
        'Best Use Cases': 'NLP, speech recognition, complex sequence modeling'
    },
    'GRU': {
        'Task': 'Text Classification',
        'Test Accuracy': gru_history['test_acc'][-1] * 100,
        'Training Time (s)': sum(gru_history['epoch_times']),
        'Parameters': sum(p.numel() for p in gru_model.parameters()),
        'Strengths': 'Faster than LSTM, good long-term memory',
        'Weaknesses': 'Slightly less expressive than LSTM',
        'Best Use Cases': 'NLP (when computational resources are limited), sequence prediction'
    },
    'DistilBERT': {
        'Task': 'Text Classification',
        'Test Accuracy': '???' , # bert_history['test_acc'][-1] * 100
        'Training Time (s)': '???' , # sum(bert_history['epoch_times'])
        'Parameters': '???' , # sum(p.numel() for p in bert_model.parameters())
        'Strengths': 'State-of-the-art NLP, fine-tunable, contextual embeddings',
        'Weaknesses': 'Computationally expensive, large parameters',
        'Best Use Cases': 'Complex NLP tasks, transfer learning'
    },
    'ViT': {
        'Task': 'Image Classification',
        'Test Accuracy': '???' , # vit_history['test_acc'][-1] * 100
        'Training Time (s)': '???' , # sum(vit_history['epoch_times'])
        'Parameters': '???' , # sum(p.numel() for p in vit_model.parameters())
        'Strengths': 'Breaks CNN inductive bias, global context, good for large datasets',
        'Weaknesses': 'Requires large datasets or pre-training, no inherent spatial bias',
        'Best Use Cases': 'Large-scale image recognition, medical imaging'
    },
    'Simple CNN': {
        'Task': 'Image Classification',
        'Test Accuracy': '???' , # cnn_accuracy * 100
        'Training Time (s)': '???' , # cnn_train_time
        'Parameters': '???' , # cnn_params
        'Strengths': 'Local feature extraction, translation invariance, good for small datasets',
        'Weaknesses': 'Limited global context, less flexible architecture',
        'Best Use Cases': 'Image classification on smaller datasets, object detection'
    }
}
...
accuracies = [v['Test Accuracy'] for v in results_summary.values() if isinstance(v['Test Accuracy'], (int, float))]
model_names = [k for k, v in results_summary.items() if isinstance(v['Test Accuracy'], (int, float))]

plt.bar(model_names, accuracies, color=['skyblue', 'lightcoral', 'lightgreen', 'orange', 'purple', 'grey'])
plt.xlabel('Model')
